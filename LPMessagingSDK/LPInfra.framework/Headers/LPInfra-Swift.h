...
#ifndef LPINFRA_SWIFT_H
#define LPINFRA_SWIFT_H
...
@property (nonatomic) enum UIViewContentMode conversationBackgroundImageContentMode;
...
/// Will return the last closed Dialog sorted by close date
///
/// returns:
/// Dialog
- (LPDialogEntity * _Nullable)getLastClosedDialog SWIFT_WARN_UNUSED_RESULT SWIFT_DEPRECATED_OBJC("Swift method 'LPConversationEntity.getLastClosedDialog()' uses '@objc' inference deprecated in Swift 4; add '@objc' to provide an Objective-C entrypoint");
/// return the latest created dialog for conversation sort by  dialog.creationDate
///
/// returns:
/// latest dialog
...
/// The files will be get deleted according to FIFO methodology.
...
/// Clear a single conversation and its associated messages and files.
...
///
/// returns:
/// If conversation doesn’t exist in DB nil will be returned
...
///
/// \param completion completion block with User instance
///
...
/// Attach completion block which is invoked when the Consumer (My) User is retrieved
...
/// Handle agent details that fetched and notify for fetched user
...
///     When all params available - perform register
///   </li>
/// </ul>
...
///   </li>
/// </ul>
+ (void)unregisterPusherFor:(NSString * _Nonnull)brandId :(BOOL)isLogout completion:(void (^ _Nonnull)(void))completion failure:(void (^ _Nonnull)(NSError * _Nonnull))failure;
/// Will check if current user is registered for Push Notification on LP Pusher
/// important:
...
///
+ (void)isRegisteredForPushNotifications:(id <ConversationParamProtocol> _Nonnull)conversationQuery completion:(void (^ _Nonnull)(BOOL))completion failure:(void (^ _Nonnull)(NSError * _Nonnull))failure;
...
/// This is for use within the LPMessagingSDK ONLY!  This API Will not be Honored for EXTERNAL USE
/// note:
/// This method requires the user to accept/allow push notifications (dependency of pusher)
/// attention:
/// This method replaces the old implementation of unreadMessageCount.getUnreadMessageCountFromPusher with new unreadMessageCount.getUnreadMessageCountFromPusherV2
/// \param conversationQuery used to identify the related brand
///
/// \param completion called once the operation ends successfully
///
/// \param failure called once the operation failed
///
...
/// This is for use within the LPMessagingSDK ONLY!  This API Will not be Honored for EXTERNAL USE
/// note:
/// This method removes dependency on the Pusher registration
/// attention:
/// This method replaces the old implementation of unreadMessageCount.getUnreadMessageCountFromPusher with new unreadMessageCount.getUnreadMessageCountFromPusherV2
...
/// Will mark conversations as read on the Pusher, synchronizing unread count on Pusher server
/// important:
/// This is for use within the LPMessagingSDK ONLY!  This API Will not be Honored for EXTERNAL USE
/// \param brandID the account number for the brand
///
/// \param conversations list of conversations to read
///
/// \param completion called once the operation ends sucessfully
///
/// \param failure called once the operation failed
///
+ (void)readAcknowledmentPusherWithBrandID:(NSString * _Nonnull)brandID conversations:(NSArray<LPConversationEntity *> * _Nonnull)conversations completion:(void (^ _Nonnull)(void))completion failure:(void (^ _Nonnull)(NSError * _Nonnull))failure;
...
/// Save a String value to the keychain associated with a specified key. If a String value already exists for the given key name, the string will be overwritten with the new value.
...
+ (NSArray<NSString *> * _Nonnull)migrateKeychainFor:(NSArray<NSString *> * _Nonnull)keychainItems SWIFT_WARN_UNUSED_RESULT;
...
/// Clear a initial created conversation and its associated messages.
...
/// Will check JWT expiration time and return true if JWT is expired
/// \param jwtToken JWT/LP_JWT
///
...
typedef SWIFT_ENUM(NSInteger, LPLoggingLevel, closed) {
  LPLoggingLevelOFF = 0,
...
typedef SWIFT_ENUM(NSInteger, LPRegionCode, closed) {
...
/// <ol>
///   <li>
///     First validation will check with the value from LPCDN (which we use for all users), if version applicable proceed.
///   </li>
///   <li>
...
///
/// \param completion a boolean in a completion block. The SDK is applicable if the current version is greater or equal to the fetched value
///
...
/// Get all supported languages by key-value (key is language code key and value is language description), call the following:
+ (NSDictionary<NSString *, NSString *> * _Nonnull)getAllSupportedLanguages SWIFT_WARN_UNUSED_RESULT;
/// Get all supported languages by key-value (key is region code key and value is region description), call the following:
...
+ (BOOL)isLocaleSupportedByCB:(NSString * _Nonnull)locale SWIFT_WARN_UNUSED_RESULT;
/// device locale code zh-Hans  and zh-Hant-HK  is not supported by controller bot as these are being new locale for zh-CN and zh-HK respectively
///
/// returns:
/// old chinese locale string
+ (NSString * _Nullable)getPreferredCBChineseLocale:(NSString * _Nonnull)locale SWIFT_WARN_UNUSED_RESULT;
...
@interface QuickReplyButton (SWIFT_EXTENSION(LPInfra))
