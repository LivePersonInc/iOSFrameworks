...
/// A boolean which determines whether to retrieve the agent details from the last closed conversation in case there is no assigned
/// agent. Agent details will be retrieved from API method: func getAssignedAgent(conversationQuery: ConversationParamProtocol)
...
/// Array of images for creating the custom refresh controller. The custom refresh controller will loop the images from the array. It
/// will need two or more images in the array for loop to take effect.
...
/// When not nil, use this attribute as the conversation portrait background image. When an image is shown, it is recommended to set
/// ‘dateSeparatorBackgroundColor’ config to clear.
...
/// attention:
/// if <em>enableConversationSeparator</em> is set to <em>false</em>, Conversation Separator Text won’t be displayed even if this
/// property is enable
...
/// attention:
/// if <em>enableConversationSeparator</em> is set to <em>false</em>, Conversation Separator Line won’t be displayed even if this
/// property is enable
...
/// attention:
/// Setting this property to false will also disable <em>enableConversationSeparatorTextMessage</em> &
/// <em>enableConversationSeparatorLine</em>
...
/// Upon SDK initialization, all closed conversation with end date older than X months, will be deleted from the database. Setting 0
/// will delete all closed conversation.
...
/// If true, then new message won’t interrupt the previous ongoing voice over announcement rather it will be queued to be announced
/// sequentially.
@property (nonatomic) BOOL announceEventsSequentially;
/// If false, then new message content won’t be announced rather it will just announce ‘New Message’
@property (nonatomic) BOOL announceNewMessageContent;
...
/// Enabling this flag, SDK will use regular expression defined in ‘clientOnlyMaskingRegex’ to mask text on the consumer device.  This
/// is client only masking because it is both set and executed within the consumers device. All masked data will appear as asterisks,
/// will be saved to local db masked and will be sent to the server unmasked. This feature is only available for Unauthenticated
/// conversations.  Default = false.
@property (nonatomic) BOOL enableClientOnlyMasking;
/// Enabling this flag, SDK will use regular expression defined in ‘realTimeMaskingRegex’ to mask text originated on the consumer
/// device.  This masking is applied to messages both on the consumer device and sent to the agent. All masked data will appear as
/// asterisks, will be saved to local db masked and will be sent to the server masked. This feature is only available for
/// Unauthenticated conversations.  Default = false.
...
/// Region - when it is not nil, it will be combined with ‘language’ (”<language>_<country>”, for example: en_US) and used instead of
/// device default locale .
/// The combined value have to be a part of iOS available Locale identifiers (use Locale.availableIdentifiers to validate). Otherwise,
...
///   </li>
///   <li>
///     will be combined with ‘country’ (“language_country”, for example: en_US) and used instead of default device locale when
///     formatting time and date.
///     If no value is provided, the SDK will use the device’s language.
///   </li>
/// </ol>
@property (nonatomic) enum LPLanguage language;
/// Bool used to allow manual configuration for enabling the SDK to register to LP Pusher service. Requires user logout to change
/// current configuration. Default is set to true.
@property (nonatomic) BOOL enableLpPusherService;
/// Bool used to clear unread badge count for all the unread messages of all the conversations at once on Pusher whenever user comes to
...
/// This will allow to adjust the jpeg compression on resize image function when consumer sends to agent. Lower = more higher = less.
/// Must be between 0 and 1.
@property (nonatomic) CGFloat maxPhotoCompression;
/// This will allow the image resize to be increased for the image height when consumer sends to agent.  Must not be more than 1600 or
/// less than 0.
@property (nonatomic) CGFloat maxPhotoHeightSize;
/// This will allow the image resize to be increased for the image width when consumer sends to agent. Must not be more than 1600 or
/// less than 0.
...
/// NOTE: this property gets its tint color from <code>photosharingMenuButtonsTintColor</code> and background color from
/// <code>photosharingMenuButtonsBackgroundColor</code>
...
/// The image must conforms Apple’s <a href="https://developer.apple.com/ios/human-interface-guidelines/icons-and-images/custom-icons/">Custom Icon
/// guidelines</a>.
...
/// Structured Content Latitude Delta Span. Used to determine which area of the map to focus. If you set this attribute, you must set
/// <code>structuredContentMapLongitudeDeltaSpan</code> as well. This parameter is used to create an
/// <a href="https://developer.apple.com/documentation/mapkit/mkcoordinatespan">MKCoordinateSpan</a>.
@property (nonatomic) double structuredContentMapLatitudeDeltaDeltaSpan;
/// Structured Content Longitude Delta Span. Used to determine which area of the map to focus. If you set this attribute, you must set
/// <code>structuredContentMapLatitudeDeltaDeltaSpan</code> as well. This parameter is used to create an
/// <a href="https://developer.apple.com/documentation/mapkit/mkcoordinatespan">MKCoordinateSpan</a>.
...
/// attention:
/// Accessibility compliance highlights the need for links and buttons to have a distinctive UI characteristic. Changing
/// this value will break accessibility compliance.
...
/// Expiration of CSAT in minutes from the moment the conversation was ended. If Survey exceeded the expiration, it will not be
...
/// When set to <em>true</em>, the TTR notifications show with off hours.  When the auto messages feature is enabled, TTR notifications do
/// not display when the auto messages featuer is enabled, regardless of the value set for this attribute.
...
/// Off Hours banner timezone based on NSTimeZone names. If sending empty string, the local timezone will be used (Server sends UTC
/// time)
...
/// important:
/// As of SDK 5.1 & above, This configuration no longer impacts the scroll of the conversation or the scroll to bottom
/// button (message preview or count indicator)
/// note:
...
/// will not be displayed. Regardless of <em>unreadMessagesDividerEnabled</em> value, the conversation screen will always scrolls to the last
/// position where the user left off.
...
/// Color of the remote user’s bubble overlay when user use long press gesture on the bubble. Overlay will appear as long as the menu
...
/// Color of the user bubble’s overlay when user use long press gesture on the bubble. Overlay will appear as long as the menu
/// controller appears on the bubble, when the menu dismissed, overlay will disappear too. In order to show overlay
/// enableBubblesOverlayOnLongPress should be true.
@property (nonatomic, strong) UIColor * _Nonnull userBubbleLongPressOverlayColor;
/// Alpha of the user’s bubble overlay when user use long press gesture on the bubble. Value can be 0.0 - 1.0. Overlay will appear as
/// long as the menu controller appears on the bubble, when the menu dismissed, overlay will disappear too. In order to show overlay
/// enableBubblesOverlayOnLongPress should be true.  Default value is Default value is 0.3.
...
/// Regular expression for email hyperlinks in users messages (consumer and agent). This attribute is optional - If not assigned, the
/// default link detection will be enabled.
@property (nonatomic, copy) NSString * _Nonnull bubbleEmailLinksRegex;
/// Regular expression for url hyperlinks in users messages (consumer and agent). This attribute is optional - If not assigned, the
/// default link detection will be enabled.
@property (nonatomic, copy) NSString * _Nullable bubbleUrlLinksRegex;
/// Regular expression for phone hyperlinks in users messages (consumer and agent). This attribute is optional - If not assigned, the
/// default link detection will be enabled.
...
/// When true, user and remote user messages containing one or two emojies will be enlarged in chat. Messages with one emoji will be the
/// largest, two emojis will be large, and 3 or more will be displayed as normal text.
...
/// When pressed, the
/// <a href="mobile-app-messaging-sdk-for-ios-sdk-apis-callbacks-index.html#lpmessagingsdkcustombuttontapped">LPMessagingSDKCustomButtonTapped</a>
...
/// Color for hyperlink in the text of the remote user bubble.
@property (nonatomic, strong) UIColor * _Nonnull remoteUserBubbleHyperlinkColor;
/// Color for hyperlink in the text of the bot user bubble.
@property (nonatomic, strong) UIColor * _Nonnull botUserBubbleHyperlinkColor;
...
/// Allow URL Protocol sniffing via the use of HTTP request with URLSession shared instance. NOTE: When setting this configuration TRUE
/// <ul>
///   <li>
///     Cert Pinning option will be disabled!
///   </li>
/// </ul>
...
/// CSDS Domain URL.  For brands that need to control the URL that is the gateway for LivePerson services, use this key to set a URL of
/// your choice.
@property (nonatomic, copy) NSString * _Nonnull csdsDomain;
/// lpTag Domain URL.  For brands that need to control the URL that is the gateway for LivePerson services, use this key to set a URL of
/// your choice.
...
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
/// Calling this will reset the configurations to default values… not all values will be updated based on how the respective managers
/// are instantiated.
- (void)resetToDefaultConfiguration;
/// Fallback in case setValueForKey failed. This method is necessary as the host app might call a deprecated value which will cause an
/// NSUnknownKeyException if this method is not implemented.
...
/// - Code Challenge
/// - Code Verifier
- (void)getPkceParamsWithAccountID:(NSString * _Nonnull)accountID completion:(void (^ _Nonnull)(LPPKCEResponse * _Nonnull))completion failure:(void (^ _Nonnull)(NSError * _Nonnull))failure;
...
///
- (LPNotification * _Nullable)handlePush:(NSDictionary * _Nonnull)userInfo;
...
@protocol LPMessagingSDKNotificationDelegate
@optional
- (void)LPMessagingSDKNotificationWithDidReceivePushNotification:(LPNotification * _Nonnull)notification SWIFT_AVAILABILITY(ios,deprecated=0.0.1,message="This delegate method id deprecated as LPNotification object is being return from handlePush(:) method.");
...
enum LPNotificationType : NSInteger;
...
@property (nonatomic, readonly) enum LPNotificationType notificationType;
...
typedef SWIFT_ENUM(NSInteger, LPNotificationType, open) {
  LPNotificationTypeRegular = 0,
  LPNotificationTypeProactive = 1,
  LPNotificationTypeC2m = 2,
};
...
SWIFT_CLASS("_TtC14LPMessagingSDK14LPPKCEResponse")
@interface LPPKCEResponse : NSObject
@property (nonatomic, copy) NSString * _Nonnull codeVerifier;
@property (nonatomic, copy) NSString * _Nonnull codeChallenge;
@property (nonatomic, copy) NSString * _Nonnull codeChallengeMethod;
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
@property (nonatomic) BOOL presented;
@property (nonatomic) BOOL isMaskedContent;
